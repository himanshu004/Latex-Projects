...
\usepackage[vmargin=30mm,hmargin=42mm]{geometry}
\thispagestyle{empty} 
\begin{document}
\begin{flushright}
...
\begin{enumerate}
    ...
    \begin{description}
	    \item
	    [\bf Windows] Click on the graphics window and select \texttt{file} $\rightarrow$ \texttt{save as}  $\rightarrow$\\ \texttt{pdf}.
    \end{description}
    \begin{description}
	    \item[linux/UNIX] 
	        Use the {\em R} command \texttt{dev.copy2pdf(file="filename.pdf").}\\ (Note that this also works under Windows.)
    \end{description}
    \item 
        Equations, such as \begin{displaymath}
